...
{
	float32_t norm_acc=0.0f, norm_mag=0.0f;
	arm_status arm_status_temp;
	
	arm_status_temp = arm_sqrt_f32((float32_t)icm_data.x_accel * icm_data.x_accel + (float32_t)icm_data.y_accel * icm_data.y_accel + icm_data.z_accel * icm_data.z_accel, &norm_acc);
	...
	arm_status_temp = arm_sqrt_f32((float32_t)icm_data.x_magnet * icm_data.x_magnet
								+ (float32_t)icm_data.y_magnet * icm_data.y_magnet + (float32_t)icm_data.z_magnet * icm_data.z_magnet, &norm_mag);
	...
	imu->az = ((uint32_t)icm_data.z_accel) / norm_acc;
