...
{
	uint64_t temp_counter = __HAL_TIM_GET_COUNTER(encoder -> htim_encoder);
	...
	else
	{
		...
		else
		{
			...
			{
				encoder -> velocity = scaling * (temp_counter + (__HAL_TIM_GET_AUTORELOAD(encoder -> htim_encoder) - encoder -> last_counter_value));
			}
